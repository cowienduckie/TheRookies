export {
  CreateCategoryPage,
  loader as CreateCategoryLoader,
  action as CreateCategoryAction,
} from "./CreateCategoryPage";
export {
  UpdateCategoryPage,
  loader as UpdateCategoryLoader,
  action as UpdateCategoryAction,
} from "./UpdateCategoryPage";
export { action as DeleteCategoryAction } from "./DeleteCategoryPage";
export {
  ViewCategoryListPage,
  loader as ViewCategoryListLoader,
} from "./ViewCategoryListPage";
export {
  CategoryManagePage,
  loader as CategoryManageLoader,
} from "./CategoryManagePage";
